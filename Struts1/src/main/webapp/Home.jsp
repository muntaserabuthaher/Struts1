...
<%int idX =(int)resultSet.getInt("id"); %>
	<form action="ChangeTaskServlet" id="changStatus" method="get">
		<tr bgcolor="#DEB887">
			...
			<td><select onchange="changeState()" name="status">
					<option value="In-Progress,<%=resultSet.getInt("id") %>"
						name="In-Progress"
						<%if(resultSet.getString("status").equals("In-Progress")){%>
						selected <%}%>>In-Progress</option>
					<option value="Completed,,<%=resultSet.getInt("id") %>"
						name="Completed"
						<%if(resultSet.getString("status").equals("Completed")){%>
						selected <%}%>>Completed</option>
					<option value="Done,,<%=resultSet.getInt("id") %>" name="Done"
						<%if(resultSet.getString("status").equals("Done")){%> selected
						<%}%>>Done</option>
			</select></td>
